 ...
 */
public abstract class AbstractServerConfiguration implements IServerConfiguration {
  ...
  /**
   * {@link AbstractServerConfiguration} constructor.
   *
   * @param path
   */
  protected AbstractServerConfiguration(IFolder path) {
    configPath = path;
  }
  ...
  public IStatus backupAndPublish(IPath destDir, IProgressMonitor monitor) {
    ...
    try {
      IPath backup = null;
      backupFolder(configPath, destDir.append("conf"), backup, ms, monitor);
    } catch (Exception e) {
