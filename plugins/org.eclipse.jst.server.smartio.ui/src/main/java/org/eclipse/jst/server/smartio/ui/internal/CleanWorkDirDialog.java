...
import org.eclipse.jst.server.smartio.core.internal.IServerWebModule;
import org.eclipse.jst.server.smartio.core.internal.ServerBehaviour;
...
public class CleanWorkDirDialog extends Dialog {
    ...
    protected IStatus run(IProgressMonitor monitor) {
      ...
      if (state != IServer.STATE_STOPPED) {
        ...
        while (!done) {
          ...
          // If forced, or there was an error (doesn't include timeout), or we are stopped, time to
          // exit
  ...
  private class DeleteWorkDirJob extends Job {
    ...
    protected IStatus run(IProgressMonitor monitor) {
      ...
      ServerBehaviour tsb = (ServerBehaviour) server.loadAdapter(ServerBehaviour.class, monitor);
      try {
        if (webModule != null) {
          IServerWebModule tcWebModule = new WebModule(webModule.getContextRoot(), "", "", true);
