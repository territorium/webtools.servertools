...
import org.eclipse.jst.server.smartio.ui.internal.Trace;
...
class WebModuleDialog extends Dialog {
  ...
  private final IServerAttributes server;
   ...
   */
  WebModuleDialog(Shell parentShell, IServerAttributes server, WebModule module) {
    super(parentShell);
    this.module = module;
    this.server = server;
    isEdit = true;
   ...
   */
  WebModuleDialog(Shell parentShell, IServerAttributes server, boolean isProject) {
    this(parentShell, server, new WebModule("/", "", null, true));
    isEdit = false;
      ...
      if (modules != null) {
        ...
        for (int i = 0; i < size; i++) {
          ...
          if ("jst.web".equals(module3.getModuleType().getId())) {
            IStatus status = server.canModifyModules(new IModule[] { module3 }, null, null);
    ...
    if (!isProject) {
      ...
      reloadable.addSelectionListener(new SelectionAdapter() {
        ...
        public void widgetSelected(SelectionEvent e) {
          module =
              new WebModule(module.getPath(), module.getDocumentBase(), module.getMemento(), reloadable.getSelection());
        }
      });
